...
`/triage` and the PR skills use role names. This file maps each role to the label string in this repository's tracker, for an issue and for a PR. The set is owned by `skills/in-progress/setup-github-labels/labels.json`.
